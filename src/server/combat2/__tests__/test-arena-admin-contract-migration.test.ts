...
describe('Combat2 arena admin contract',()=>{
 ...
 it('keeps the UI RPC-only, guarded by the existing admin route and exact confirmations',()=>{
  ...
  expect(UI).toContain('Stop Recording and generate report');
  expect(API).toContain('_confirm_destroy_diagnostics:true');
  expect(UI).toContain('disabled={recording||!status?.resetEligible||status.activePresenceCount>0||status.arenaLiveClaimCount>0||!!busy}');
  ...
  expect(API.match(/combat2_test_[a-z_]+/g)?.every(name=>['combat2_test_status','combat2_test_runtime_status','combat2_test_grant','combat2_test_revoke','combat2_test_admin_relocate','combat2_test_stop','combat2_test_emergency_shutdown','combat2_test_reset','combat2_test_environment_start','combat2_test_environment_close','combat2_test_run_start','combat2_test_run_stop','combat2_test_run_report'].includes(name))).toBe(true);

            ...
            <AccordionContent className="px-4">
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  <p><strong className="text-foreground"> Rend (T3, 40 CP):</strong> Bleed = <code className="text-primary">floor((softScale(STR_mod, 'dot') × 1.5 + 2) × 0.67)</code> per 2s tick (STR, soft-scaled past +20), Duration = <code className="text-primary">min(30s, 20s + DEX_mod × 1s)</code> (DEX keeps the wound open). <em className="text-muted-foreground">Multi-target: can bleed multiple creatures simultaneously (tracked per creature).</em></p>
                  <p><strong className="text-foreground"> Sunder Armor (T4, 60 CP):</strong> AC Reduction = <code className="text-primary">round(2 + softScale(STR_mod, 'utility'))</code> (STR, soft-scaled past 12), Duration = <code className="text-primary">min(20s, 12s + DEX_mod)</code>s (DEX = precise strike, lasting weakness)</p>
                  ...
                  <p><strong className="text-foreground"> Grand Finale (T4, 60 CP):</strong> Dmg = <code className="text-primary">max(8, softScale(CHA_mod, 'burst') × 4 + floor(level × 1.5)) + 1d(softScale(CHA_mod, 'burst') × 2)</code> (CHA soft-scaled past +18)</p>

                  <p className="text-[10px] font-semibold text-primary/70 mt-2">— Templar —</p>
                  <p><strong className="text-foreground"> Judgment (T0, 10 CP):</strong> Holy damage = <code className="text-primary">floor((5 + 2·WIS_mod + floor(level/3)) × 0.8)</code> — 20% reduced vs the shared smite baseline for balance.</p>

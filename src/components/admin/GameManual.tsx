            ...
            <AccordionContent className="px-4">
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  <p><strong className="text-foreground">🔨 Sunder Armor (T4, 60 CP):</strong> <span className="text-dwarvish">⏳</span> AC Reduction = <code className="text-primary">max(2, STR_mod)</code> (STR), Duration = <code className="text-primary">min(20s, 12s + DEX_mod)</code>s (DEX = precise strike, lasting weakness)</p>
                  ...
                  <p><strong className="text-foreground">🌋 Ignite (T3 stance, 50 CP):</strong> <span className="text-soulforged">⚓</span> Reserves 20% of max CP. While in combat, each heartbeat an orb has a 40% chance to strike your target for <code className="text-primary">2 + INT_mod</code> direct damage and apply a burn — <code className="text-primary">floor(WIS_mod × 0.7 × 0.67)</code> damage per stack per 2s tick (max 5 stacks). Duration <code className="text-primary">30s + WIS_mod × 1s</code> (cap 45s). INT shapes the spark, WIS sustains the flame. <em className="text-muted-foreground">Mutually exclusive with Envenom. Persists until dropped — CP not refunded.</em></p>
                  ...
                  <p><strong className="text-foreground">🛡️ Shield Wall (T2 Stance, 15% max CP reserved):</strong> <span className="text-soulforged">⚓</span> Dual-primary scaling — WIS adds bonus block chance (+30% floor, scaling up to +55% at high WIS); CON adds bonus block amount (+5 floor, scaling up to +11 at high CON). Stacks on top of base block (DEX→chance, STR→amount); final chance capped at 95%. <strong>Requires a shield equipped.</strong> Click again to drop — reserved CP is NOT refunded.</p>
            ...
            <AccordionContent className="px-4 space-y-3">
              ...
              {isValar && (
                <div className="mt-3 border border-primary/30 rounded bg-primary/5">
                  <div className="flex items-center gap-2 px-3 py-2 border-b border-primary/30">
                    <Wrench className="w-3 h-3 text-primary" />
                    <span className="text-xs font-display text-primary">Overlord Tuning · Loot Pool Rules</span>
                  </div>
                  <PoolRulesTab />
                </div>
              )}

...
import { Search, KeyRound, Shield, Ban, UserCheck, ChevronDown, ChevronUp, Pencil, Save, X } from 'lucide-react';
...
export default function UserManager({ isValar }: Props) {
  ...
  const handleSaveCharacter = async (charId: string) => {
    try {
      await callAdmin('update-character', 'POST', {
        character_id: charId,
        updates: charEdits,
      });
  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-3 overflow-y-auto">
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Search by email, name, or character..."
      ...
      <div className="space-y-1">
        {loading ? (
          <p className="text-xs text-muted-foreground text-center py-8">Loading users...</p>
          ...
          return (
            <div key={u.id} className="border border-border rounded bg-card/50">
              {/* User row */}
              <div
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-display text-foreground truncate">
                ...
                <div className="text-right shrink-0">
                  <p className="text-[10px] text-muted-foreground">
                    {u.characters.length} char{u.characters.length !== 1 ? 's' : ''}
                  </p>
                  <p className="text-[10px] text-muted-foreground">
                    Last: {formatDate(u.last_sign_in_at)}
                  </p>
                </div>
              ...
              {expanded && (
                <div className="border-t border-border px-3 py-2 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-[10px]">
                    ...
                    <span>{u.email_confirmed_at ? 'Yes' : 'No'}</span>
      ...
      {total > 50 && (
        <div className="flex justify-center gap-2">
          ...
          <Button size="sm" variant="outline" className="h-7 text-xs"
            disabled={users.length < 50} onClick={() => setPage(p => p + 1)}>
            Next
          </Button>
        </div>
      )}

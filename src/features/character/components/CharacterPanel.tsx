              ...
              <div className="gap-group.5">
                ...
                {(() => {
                  ...
                  const offenseRows: DerivedRow[] = [
                    ...
                    { label: 'Hit Chance', value: `${hitChance}%`, tip: `d20 + ${proficiency} proficiency + ${hitMod} DEX accuracy${insightBonus ? ` + ${insightBonus} INT Combat Insight` : ''}${affinityHit ? ' + 1 Affinity' : ''} → ${hitChance}% vs same-level creature (AC ${sameLevelAC})\n(Autoattack damage scales from STR; accuracy from DEX. INT adds Combat Insight to every attack that is not INT-based. Each ability names its own accuracy attribute.)` },
                    { label: 'Combat Insight', value: insightBonus > 0 ? `+${insightBonus}` : '–', tip: insightBonus > 0 ? `INT adds +${insightBonus} to-hit on attacks whose accuracy attribute is not INT (max +5). INT-based abilities count INT once, through their own accuracy bonus.` : 'INT 12+ grants Combat Insight: a to-hit bonus on every attack that is not INT-based (max +5).' },
                ...
                <ClassBondRow
                  ...
                  activeClass={(character as any).class ?? null}
                />

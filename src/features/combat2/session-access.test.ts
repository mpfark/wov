...
describe('Combat2 self-access adapter',()=>{
 ...
 it('preserves only known safe refusal/error classifications',()=>{
  ...
  expect(decodeSessionAccess({ok:false,kind:'not_enabled'},node)).toEqual({status:'refused',classification:'not_enabled'});
  expect(decodeSessionAccess({ok:false,kind:'mode_refused'},node)).toEqual({status:'refused',classification:'mode_refused'});
 ...
 it('calls the exact self-scoped RPC without a user id and redacts transport errors',async()=>{
  ...
  expect(rpc).toHaveBeenCalledWith('combat2_session_access',{_character_id:character,_node_id:node});
 });
});

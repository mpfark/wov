...
export type SessionAccessResult =
  | { status:'allowed'; scope:'test_arena'|'ordinary_world'|'canary'; nodeId:string }
...
export function decodeSessionAccess(value:unknown, requestedNodeId:string):SessionAccessResult {
  ...
  if(value.ok===true&&value.kind==='allowed'&&(value.scope==='test_arena'||value.scope==='ordinary_world'||value.scope==='canary')&&
...
type WorldPresenceClient={rpc(name:'combat2_presence_heartbeat',args:{_character_id:string}):PromiseLike<{data:unknown;error:{message?:string}|null}>};
export async function heartbeatCombat2Presence(characterId:string,client:WorldPresenceClient={rpc:(name,args)=>supabase.rpc(name as never,args as never)}):Promise<boolean>{
 try{const {data,error}=await client.rpc('combat2_presence_heartbeat',{_character_id:characterId});return !error&&object(data)&&data.ok===true&&data.kind==='present';}catch{return false;}
}

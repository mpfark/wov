...
describe('real party UI authority',()=>{
 ...
 it('keeps unsupported following visibly refused and performs no direct party mutation',()=>{expect(HOOK).toContain('Party follow is not connected to the authoritative party system yet.');expect(HOOK).not.toMatch(/\.from\(['"](?:parties|party_members)['"]\)/);expect(HOOK).not.toContain('accept_party_invite');expect(HOOK).not.toContain('set_party_tank');});
});

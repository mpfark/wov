...
export const CLASS_ABILITIES: Record<string, ClassAbility[]> = {
  ...
  warrior: [
    { label: 'Power Strike', emoji: '⚔️', description: 'A heavy, focused blow that deals damage scaling with STR', tooltip: 'Heavy single-target blow. Scales with STR.', cpCost: 10, type: 'power_strike', tier: 0, levelRequired: 1 },
    ...
    { label: 'Battle Cry', emoji: '📯', description: '⚓ Stance (T2 — reserves 15% of max CP). Reduces incoming damage by 15% (20% with shield) and softens crits while active. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Reduce incoming damage and soften crits. Stance.', cpCost: 25, type: 'battle_cry', tier: 2, levelRequired: 10 },
    { label: 'Rend', emoji: '🩸', description: 'Slice your target, applying a bleed that deals STR-scaled damage every 2s. Duration scales with DEX (precision keeps the wound open).', tooltip: 'Bleed your target over time. Damage scales with STR, duration with DEX.', cpCost: 40, type: 'dot_debuff', tier: 3, levelRequired: 15 },
  ...
  ranger: [
    { label: 'Aimed Shot', emoji: '🎯', description: 'Take a careful shot at your target, scaling with DEX', tooltip: 'Careful single-target shot. Scales with DEX.', cpCost: 10, type: 'aimed_shot', tier: 0, levelRequired: 1 },
    { label: 'Eagle Eye', emoji: '🦅', description: '⚓ Stance (T1 — reserves 10% of max CP). Widens your critical hit range based on a blend of DEX (precision) and WIS (attunement) while active. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Widen your crit range. Scales with DEX and WIS. Stance.', cpCost: 15, type: 'crit_buff', tier: 1, levelRequired: 5 },
    { label: 'Barrage', emoji: '🏹', description: 'Fire a volley of arrows at 70% damage each (scales with DEX). Arrow count: 2 base, +1 with DEX≥3, +1 more with WIS≥4 (max 4).', tooltip: 'Fire a volley of arrows. Damage scales with DEX, arrow count with WIS.', cpCost: 25, type: 'multi_attack', tier: 2, levelRequired: 10 },
    { label: "Nature's Snare", emoji: '🌿', description: 'Entangle your target, reducing its damage by 30%. Duration scales with WIS.', tooltip: "Reduce target's damage by 30%. Duration scales with WIS.", cpCost: 40, type: 'root_debuff', tier: 3, levelRequired: 15 },
    { label: 'Disengage', emoji: '🦘', description: 'Leap backward — dodge all attacks briefly and deal 50% bonus damage on your next strike', tooltip: 'Dodge all attacks briefly; next strike deals +50% damage.', cpCost: 60, type: 'disengage_buff', tier: 4, levelRequired: 20 },
  ],
  bard: [
    ...
    { label: 'Dissonance', emoji: '💢', description: "A discordant note that reduces your target's damage by 30%. Duration scales with INT (bards' attunement to the rhythm).", tooltip: "Reduce target's damage by 30%. Duration scales with INT.", cpCost: 25, type: 'root_debuff', tier: 2, levelRequired: 10 },
  ...
  rogue: [
    { label: 'Backstab', emoji: '🗡️', description: 'Strike at a vital point for damage scaling with DEX', tooltip: 'Strike a vital point. Scales with DEX.', cpCost: 10, type: 'backstab', tier: 0, levelRequired: 1 },
    ...
    { label: 'Envenom', emoji: '🐍', description: '⚓ Stance (T3 — reserves 20% of max CP). Each hit has a 40% chance to apply a stackable poison DoT (max 5). Mutually exclusive with Ignite. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Hits may apply stacking poison. Stance.', cpCost: 50, type: 'poison_buff', tier: 2, levelRequired: 10 },
    { label: 'Eviscerate', emoji: '🔪', description: 'A vicious strike that consumes all poison stacks for bonus damage per stack. Per-stack bonus scales with CHA (showmanship per stack), capped at +65% per stack.', tooltip: 'Consume poison stacks for bonus damage. Per-stack bonus scales with CHA.', cpCost: 40, type: 'execute_attack', tier: 3, levelRequired: 15 },
    { label: 'Cloak of Shadows', emoji: '🌫️', description: 'Wrap yourself in shadow, gaining a 50% chance to dodge incoming attacks', tooltip: '50% chance to dodge incoming attacks.', cpCost: 60, type: 'evasion_buff', tier: 4, levelRequired: 20 },
  ],
  wizard: [
    ...
    { label: 'Force Shield', emoji: '🛡️', description: '⚓ Stance (T1 — reserves 10% of max CP). Maintains an arcane absorb shield (WIS-scaled pool, INT-scaled regen) that re-forms out of combat. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Maintain an arcane absorb shield. Pool scales with WIS, regen with INT. Stance.', cpCost: 15, type: 'absorb_buff', tier: 1, levelRequired: 5 },
    { label: 'Arcane Surge', emoji: '✨', description: '⚓ Stance (T2 — reserves 15% of max CP). All your damage is increased by 15% while active. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Increase all your damage by 15%. Stance.', cpCost: 25, type: 'damage_buff', tier: 2, levelRequired: 10 },
    { label: 'Ignite', emoji: '🌋', description: '⚓ Stance (T3 — reserves 20% of max CP). While in combat, each heartbeat an orb has a 40% chance to strike your target — INT-scaled spark damage that also applies a WIS-scaled stackable burn (max 5, longer with more WIS). Mutually exclusive with Envenom. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Orbs strike your target and apply stacking burn. Spark scales with INT, burn with WIS. Stance.', cpCost: 50, type: 'ignite_buff', tier: 3, levelRequired: 15 },
    { label: 'Conflagrate', emoji: '💥', description: 'Consume all burn stacks on your target for +50% bonus damage per stack', tooltip: 'Consume burn stacks for +50% damage per stack.', cpCost: 60, type: 'ignite_consume', tier: 4, levelRequired: 20 },
  ],
  templar: [
    ...
    { label: 'Holy Shield',      emoji: '⚡',  description: '⚓ Stance (T1 — reserves 10% of max CP). Attackers who strike you take holy damage in return — magnitude scales with WIS, with a CON kicker (CON adds to retaliation damage). Once per attacker per tick. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Attackers take holy damage in return. Scales with WIS, CON adds a kicker. Stance.', cpCost: 15, type: 'reactive_holy', tier: 1, levelRequired: 5 },
    { label: 'Shield Wall',      emoji: '🛡️', description: '⚓ Stance (T2 — reserves 15% of max CP). Dual-primary: WIS adds bonus block chance (+30% floor, up to +55% at high WIS), CON adds bonus block amount (+5 floor, up to +11 at high CON). Final block chance capped at 95%. Requires a shield equipped to benefit. Click again to drop. Reserved CP is NOT refunded.', tooltip: 'Boost block chance and amount. Chance scales with WIS, amount with CON. Stance.', cpCost: 25, type: 'block_buff', tier: 2, levelRequired: 10 },
    ...
    { label: 'Divine Challenge', emoji: '⚜️', description: 'Take 30% less damage from all sources. Duration scales with CON (30s base, up to 45s).', tooltip: 'Take 30% less damage. Duration scales with CON.', cpCost: 60, type: 'mitigation_buff', tier: 4, levelRequired: 20 },
  ],
};

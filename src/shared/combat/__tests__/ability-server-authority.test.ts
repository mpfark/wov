...
beforeEach(() => {
  ...
  loader.setServerAbilityCalcs([
    row({ abilityKey: 'rend', mechanicKey: 'weapon_attack', cpCost: 12, damageType: 'physical' }),
    row({ abilityKey: 'lacerate', mechanicKey: 'weapon_attack', cpCost: 30, damageType: 'physical', isDefault: false }),
  ]);
});
...
describe('Phase 3 — authoritative cost and damage type', () => {
  ...
  it('normalizes an unknown damage type to null rather than trusting it', () => {
    loader.resetServerAbilityCalcs();
    loader.setServerAbilityCalcs([
      row({ abilityKey: 'rend', mechanicKey: 'weapon_attack', cpCost: 12, damageType: 'nonsense' }),
    ]);

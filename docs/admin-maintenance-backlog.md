...
| ADM-025A | Global unique-item identity | security / data integrity | Install the pending durable unique-instance migration, then deploy/publish the transfer and vendor guards. Inventory, ground loot and active marketplace escrow share one immutable instance identity; unique vendor stock is forbidden. | Requires aggregate Cloud compatibility preflight and official type regeneration before deployment. | Yes | Yes | Yes | Contract batch | pending installation |
| ADM-025B | Authoritative creature reward model | gameplay / admin contract | Add independent gold and per-recipient 1/2/4 salvage channels plus exclusive `none`, `world_pool`, `assigned_table`, and `unique_boss_drop` sources. Keep Combat2 unique materialization disabled until 025A is installed. Lesser unique fallback and possible return-to-seller marketplace expiry remain deferred product extensions. | Depends on ADM-025A installation and type regeneration. | Yes | Yes | Yes | Contract batch | blocked by ADM-025A |

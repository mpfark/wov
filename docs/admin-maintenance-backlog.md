...
| ADM-025A | Global unique-item identity | security / data integrity | Durable identity implementation complete; install the pending migration before ADM-025B. Inventory, ground loot and active marketplace escrow share one immutable identity; unique vendor stock is forbidden. | Requires aggregate Cloud compatibility preflight and official type regeneration. | Yes | Yes | Yes | Contract batch | implementation complete |
| ADM-025B | Authoritative creature reward model | gameplay / admin contract | Independent gold and per-recipient 1/2/4 salvage plus exclusive item sources and commit-time unique materialization implemented. Lesser unique fallback and possible return-to-seller marketplace expiry remain deferred product extensions. | Install after ADM-025A, regenerate types, deploy Combat2 Edge functions and publish frontend. | Yes | Yes | Yes | Contract batch | implementation complete |

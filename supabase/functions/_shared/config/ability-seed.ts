...
export const ABILITY_SEED: AbilitySeed[] = [
  ...
  },
  {
    ...
    target_type: 'enemy', activation_mode: 'instant', cp_cost: 40, cp_reserve_pct: null,
    amount_calc: { base: 0.25, terms: [stat('dex', 1, { clampAtZero: true, transform: { kind: 'diminishing_float', perPoint: 0.02, cap: 0.15 } })], floor: null, cap: null, unit: 'percent', note: 'DEX-scaled outgoing-damage reduction' },
    duration_calc: { base: 8000, terms: [stat('wis', 1000, { clampAtZero: true })], cap: 15000, unit: 'ms', note: 'WIS duration' },
    interval_ms: null,
    effect_config: { control_mode: 'damage_reduction', magnitude_stat: 'dex', duration_stat: 'wis' },
  ...
  },
  {
    ...
    amount_calc: { base: 0.25, terms: [stat('cha', 1, { clampAtZero: true, transform: { kind: 'diminishing_float', perPoint: 0.02, cap: 0.15 } })], floor: null, cap: null, unit: 'percent', note: 'CHA-scaled outgoing-damage reduction' },
    duration_calc: { base: 8000, terms: [stat('int', 1000, { clampAtZero: true })], cap: 15000, unit: 'ms', note: 'INT duration' },
    interval_ms: null,
    effect_config: { control_mode: 'damage_reduction', magnitude_stat: 'cha', duration_stat: 'int' },
    combat_text: { activate_text: "{ability}! {target}'s damage reduced by {pct}% for {seconds}s." },
  ...
  },
  {
    ...
    interval_ms: null,
    effect_config: { mitigation_mode: 'flat', resolved_by: 'combat-tick' },
    combat_text: {

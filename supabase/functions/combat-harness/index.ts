/**
 * combat-harness — TEMPORARY validation-only endpoint (run id: c5t20260818a).
 *
    ...
    switch (section) {
      ...
      case "fixtures": {
        ...
        const { data: character, error: charErr } = await db.from("characters").insert({
          user_id: created.user.id, name: "Fledsourceb", race: "human", class: "warrior",
        ...
        const { data: creature, error: creatureErr } = await db.from("creatures").insert({
          name: "Fled Effigy B", description: "Temporary validation fixture.", node_id: nodeId,

 ...
 *
 * Two bounded runs are allowed:
 *   - `c5p20260818b` — browser-driver preflight (Rend + movement), disposable.
 *   - `c5t20260818b` — the fled-DoT reward scenario.
 *
...
const RUNS = ["c5t20260818f"] as const;
    ...
    switch (section) {
      ...
      case "fixtures": {
        ...
        const { data: creature, error: creatureErr } = await db.from("creatures").insert({
          name: typeof body?.creature_name === "string" ? body.creature_name : "Fled Effigy",

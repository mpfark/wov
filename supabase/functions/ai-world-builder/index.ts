...
- Nodes do NOT need unique names — they inherit their area name. Only give a node a name if it's a special/notable location (inn, vendor, blacksmith, boss lair, etc.)`;
    ...
    const systemPrompt = `You are a high-fantasy world builder for a text-based RPG called "Wayfarers of Varneth". You generate regions, areas, nodes, creatures, and NPCs that fit the world's lore. Generate original names and content inspired by classic high-fantasy settings but NOT taken directly from any copyrighted works (e.g. do not use names from Tolkien, D&D, or other franchises).

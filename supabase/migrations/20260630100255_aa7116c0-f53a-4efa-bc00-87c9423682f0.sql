...
BEGIN
  ...
  -- Mirror src/shared/formulas/items.ts → effectiveItemStats:
  --   base = stat_override (when non-empty) else items.stats
  --   plus applied_gems mapped to attributes:
  --   garnet→str, topaz→dex, emerald→con, sapphire→int, pearl→wis, amethyst→cha
  -- HP comes from base stats only; gems never grant flat HP.
  WITH equipped AS (
    SELECT
...
-- One-shot backfill: recompute persisted caps for every character using the
-- corrected aggregation. Bypasses the owns_character() check by inlining the
-- same logic against each character_id.
DO $$
DECLARE
  _id uuid;
  _char RECORD;
...
BEGIN
  FOR _id IN SELECT id FROM characters LOOP
    ...
    _new_max_hp := LEAST(GREATEST(_base_hp + _con_mod + (_char.level - 1) * 5 + COALESCE(_bonus_hp, 0), 1), 10000);

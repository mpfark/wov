...
DECLARE
  ...
  PROCEDURE_NOOP int;
BEGIN
  IF NOT public.has_role(auth.uid(), 'overlord') THEN
    RAISE EXCEPTION 'not_authorized';
  END IF;

  SELECT id INTO v_node FROM public.nodes LIMIT 1;

  -- Three distinct real users, so RLS is evaluated with genuine claims.
  ...
  -- 1. Current participant reads batches.
  ...
  -- 2. Missing tick recovery: the hole is genuinely absent, neighbours present.
  SELECT count(*) INTO v_cnt FROM public.encounter_tick_batches
   WHERE encounter_id = v_enc AND tick_number BETWEEN 96 AND 96;
  SELECT count(*) INTO v_cnt FROM public.encounter_tick_batches
  ...
  -- 3. Ordering and duplicate-free identity at the source.
  SELECT bool_and(ok) INTO v_ok FROM (
    SELECT tick_number = lag(tick_number) OVER (ORDER BY tick_number) + 1 OR lag(tick_number) OVER (ORDER BY tick_number) IS NULL OR tick_number = 97 AS ok
    FROM public.encounter_tick_batches WHERE encounter_id = v_enc
  ) s;
  SELECT count(*) INTO v_cnt FROM (
    SELECT tick_number FROM public.encounter_tick_batches WHERE encounter_id = v_enc
    GROUP BY tick_number HAVING count(*) > 1
  ) d;
  v_out := v_out || jsonb_build_array(jsonb_build_object('case','ordered_unique_ticks','pass', v_ok AND v_cnt = 0));
  ...
  -- 5. Unrelated user is denied both paths.
  ...
  -- 6. Participation ends: the trigger must mint an access grant.
  ...
  -- 7. Departed participant still reads under the grace grant.
  EXECUTE 'SET LOCAL ROLE authenticated';
  ...
  -- 8. Expired grant closes the door again.
  EXECUTE 'SET LOCAL ROLE NONE';
  ...
  -- 9. Pruned-before-return: retention removes old batches, resync still answers.
  EXECUTE 'SET LOCAL ROLE NONE';
  ...
  -- 10. Grant pruning is bounded and removes only expired rows.
...
REVOKE ALL ON FUNCTION public.c4_delivery_harness_run() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.c4_delivery_harness_run() TO authenticated, service_role;
...
BEGIN
  ...
  ELSIF e.status <> 'active' THEN
    ...
    UPDATE public.node_creature SET tank_fighter_id=NULL,engaged=false,updated_at=now() WHERE encounter_id=e.id;
    UPDATE public.node_intent SET status='rejected',reject_reason='stale_generation' WHERE encounter_id=e.id AND status='pending';
    UPDATE public.node_pending_event SET consumed_at=now(),consumed_tick=e.tick WHERE encounter_id=e.id AND consumed_at IS NULL;
  ...
  END IF;
  INSERT INTO public.node_creature(encounter_id,creature_id,spawn_seq,hp,is_alive,engaged)
    SELECT e.id,cr.id,cr.spawn_seq,GREATEST(1,COALESCE(NULLIF(cr.hp,0),cr.max_hp)),true,false
    FROM public.creatures cr WHERE cr.node_id=v_node AND cr.is_alive
    ON CONFLICT(creature_id,spawn_seq) DO NOTHING;
  SELECT pm.party_id INTO v_party FROM public.party_members pm
    WHERE pm.character_id=_character_id AND pm.status='accepted' ORDER BY pm.joined_at LIMIT 1;
  IF v_party IS NULL THEN
    SELECT COALESCE(max(generation),0)+1 INTO v_generation FROM public.node_arrival_group
      WHERE encounter_id=e.id AND party_id IS NULL;
    INSERT INTO public.node_arrival_group(encounter_id,party_id,generation) VALUES(e.id,NULL,v_generation) RETURNING id INTO v_group;
  ELSE
    SELECT id INTO v_group FROM public.node_arrival_group WHERE encounter_id=e.id AND party_id=v_party AND active FOR UPDATE;
    IF NOT FOUND THEN
      SELECT COALESCE(max(generation),0)+1 INTO v_generation FROM public.node_arrival_group WHERE encounter_id=e.id AND party_id=v_party;
      INSERT INTO public.node_arrival_group(encounter_id,party_id,generation) VALUES(e.id,v_party,v_generation) RETURNING id INTO v_group;
    END IF;
  END IF;

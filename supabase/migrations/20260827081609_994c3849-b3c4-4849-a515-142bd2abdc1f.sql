-- 1. Participation generations -------------------------------------------------
CREATE SEQUENCE IF NOT EXISTS public.encounter_participation_generation_seq;

ALTER TABLE public.encounter_participants
  ADD COLUMN IF NOT EXISTS generation bigint;

ALTER TABLE public.encounter_participants
  ALTER COLUMN generation SET DEFAULT nextval('public.encounter_participation_generation_seq');

UPDATE public.encounter_participants
SET generation = nextval('public.encounter_participation_generation_seq')
WHERE generation IS NULL;

ALTER TABLE public.encounter_participants
  ALTER COLUMN generation SET NOT NULL;

-- 2. Intake issues a new generation on insert or encounter change --------------
...
BEGIN
  ...
  -- Scope strictly to the encounter of the node being LEFT: by the time the
  -- client reports a departure the character already stands somewhere else.
  ...
  -- Ending participation is what makes re-entry a new generation, so a
  -- telegraphed cast frozen against the previous visit can never reach it.
  WITH del AS (
...
-- 4. Snapshot exposes generations and cast recovery ----------------------------
    ...
    'creatures', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'castReadyAtMs', COALESCE((
          ...
          FROM public.encounter_cast_events ce
          WHERE ce.encounter_id = _encounter_id AND ce.creature_id = cr.id), 0),

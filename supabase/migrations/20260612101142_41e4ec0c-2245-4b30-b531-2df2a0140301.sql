...
DECLARE
  ...
  _deleted_count int;
BEGIN
  -- Allow the service role (edge function, which has already verified the
  -- caller owns the character) OR a direct authenticated owner call.
  IF NOT (
    COALESCE((auth.jwt() ->> 'role'), '') = 'service_role'
    OR public.owns_character(p_character_id)
  ) THEN

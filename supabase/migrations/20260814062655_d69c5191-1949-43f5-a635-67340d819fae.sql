CREATE OR REPLACE FUNCTION public.c4_harness_caller_allowed()
RETURNS boolean
LANGUAGE sql
STABLE
SET search_path TO 'public'
AS $$
  -- Overlord admins, or a trusted server-side caller (service_role) used by the
  -- controlled validation harness runner.
  SELECT public.has_role(auth.uid(), 'overlord')
      OR current_setting('request.jwt.claim.role', true) = 'service_role'
      OR (current_setting('request.jwt.claims', true)::jsonb ->> 'role') = 'service_role'
      OR auth.uid() IS NULL AND current_user IN ('service_role', 'postgres');
$$;
...
REVOKE ALL ON FUNCTION public.c4_harness_caller_allowed() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.c4_delivery_harness_run() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.c4_harness_caller_allowed() TO authenticated, service_role;
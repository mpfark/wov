-- B2 — fold encounter tick writes into one committed transaction.
-- The signature is unchanged: all new state rides in the existing `_payload`
-- under the optional `state` key, so callers that omit it behave exactly as before.
...
BEGIN
  ...
  IF v_state ? 'characters' THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_state->'characters') LOOP
      ...
      -- Resource deltas keep going through the encounter helpers so concurrent
      -- writers (catch-up, heals) cannot lose updates.
  ...
  IF v_state ? 'effects_upsert' THEN
    ...
    SELECT * FROM jsonb_populate_recordset(NULL::public.active_effects, v_state->'effects_upsert')
  ...
  -- ───────── Cursor: cadence anchored to the ACTUAL commit time ─────────
  -- Advancing by a fixed interval let due-time and request-time diverge
  -- permanently (~1.5s of drift). Anchoring to the commit converges instead.

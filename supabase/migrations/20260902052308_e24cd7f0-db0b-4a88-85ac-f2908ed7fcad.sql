-- Safely project the frozen target contract persisted by telegraphed Combat2 casts.
-- All other delivery authorization, pagination, gap detection and projections are
-- retained verbatim from the installed combat2_sync contract.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'pendingAction', CASE WHEN nc.pending_action IS NULL THEN NULL ELSE jsonb_build_object(
      ...
      'abilityLabel', nc.pending_action->>'ability_label',
      'startedAtTick', (nc.pending_action->>'started_at_tick')::bigint,
      'resolveAtTick', (nc.pending_action->>'resolve_at_tick')::bigint,
      'targetFighterId', nc.pending_action->>'target_fighter_id',
      'targetCharacterId', nc.pending_action->>'target_character_id',
      'targetEntrySeq', (nc.pending_action->>'target_entry_seq')::bigint
    ) END

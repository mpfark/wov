...
AS $$
  SELECT '{
    ...
    "ignite_buff": {"allowed": ["pulse_damage", "burn_damage", "max_stacks"], "required": ["pulse_damage", "burn_damage", "max_stacks"], "requires": ["amount", "duration", "interval"], "supports": ["amount", "duration", "interval"]},
...
SET duration_calc = '{
      "base": 30000,
      "terms": [{"source": "stat", "stat": "wis", "mult": 1000, "clampAtZero": true}],
      "cap": 45000,
      "unit": "ms",
      "note": "WIS burn duration"
    }'::jsonb,
    interval_ms = 2000,
    mechanic_calcs = '{
      "pulse_damage": {
        "base": 2,
        "terms": [{"source": "stat", "stat": "int"}],
        "rounding": "floor",
        "floor": 1,
        "cap": null,
        "unit": "hp",
        "note": "INT orb pulse damage"
      },
      "burn_damage": {
        "base": 0,
        "terms": [{"source": "stat", "stat": "wis", "mult": 0.7, "clampAtZero": true, "transform": {"kind": "soft", "profile": "dot"}}],
        "finalMult": 0.67,
        "rounding": "floor",
        "floor": 1,
        "cap": null,
        "unit": "hp",
        "note": "WIS burn damage per tick before bond multiplier"
      },
      "max_stacks": {
        "base": 5,
        "terms": [],

...
BEGIN
 ...
 IF COALESCE((stopped->>'ok')::boolean,false)=false THEN RETURN jsonb_build_object('ok',false,'kind','close_failed'); END IF;
 ...
 SELECT count(*) INTO recent_players FROM public.characters c WHERE c.last_online>now()-interval '30 minutes'
  AND NOT EXISTS(SELECT 1 FROM public.combat2_test_arena_access x WHERE x.arena_id=_arena_id AND x.character_id=c.id AND x.user_id=c.user_id AND x.active AND x.revoked_at IS NULL)
  AND NOT EXISTS(SELECT 1 FROM public.user_roles r WHERE r.user_id=c.user_id AND r.role IN('steward','overlord'));
 IF ordinary_encounters>0 OR ordinary_claims>0 OR recent_players>0 THEN
  result:=jsonb_build_object('ok',true,'kind','arena_stopped_world_left_open','ordinaryEncounterCount',ordinary_encounters,'ordinaryLiveClaimCount',ordinary_claims,'recentOrdinaryPlayerCount',recent_players);
 ELSE

...
SET mechanic_key = 'evasion_buff',
    effect_config = coalesce(effect_config, '{}'::jsonb) || jsonb_build_object(
      'dodge_chance', 1.0,
      'next_hit_window_ms', 15000,
      'evasion_source', 'disengage'
    ),
    combat_text = coalesce(combat_text, '{}'::jsonb) || jsonb_build_object(
      'activate_text',
      'Disengage! You leap back — dodging all attacks for {seconds}s. Your next strike deals +{bonus_pct}% bonus damage!'
    ),
    updated_at = now()
WHERE mechanic_key = 'disengage_buff';

UPDATE public.abilities
SET effect_config = coalesce(effect_config, '{}'::jsonb) || jsonb_build_object(
      'evasion_source', 'cloak'
    ),
    combat_text = coalesce(combat_text, '{}'::jsonb) || jsonb_build_object(
      'activate_text', 'Cloak of Shadows! {dodge_pct}% dodge chance for {seconds}s.'
    ),
    updated_at = now()
WHERE mechanic_key = 'evasion_buff'
  AND ability_key = 'cloak_of_shadows';

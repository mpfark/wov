...
BEGIN
  ...
  RETURN jsonb_build_object(
    'present', true,
    'length', length(v_key),
    'fingerprint', left(encode(digest(v_key, 'sha256'), 'hex'), 12),
    'format', CASE
...
REVOKE ALL ON FUNCTION public.effects_catchup_credential_health() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.effects_catchup_credential_health() TO service_role;
GRANT EXECUTE ON FUNCTION public.effects_catchup_credential_health() TO supabase_read_only_user;